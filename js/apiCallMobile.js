...
function drawInGameDetail(matchId, data, trClass) {
    ...
    let score = `
        <tr class='${trClass}'>
            <td>
                ${winLoseKo(playInfo.result, playInfo.playTypeName)}
            </td>
            <td>
                ${isSecond? secondIcon : "" }
            </td>
...
function appendPlayTypeInfo(div, type, typeId, positionName, nickname) {
    ...
    if (type != null) {
        ...
        const moreAlink = `<a href='#' data-bs-toggle="modal" data-bs-target="#${modalId}" onClick="javascript:playGameList('${positionName}', null, '${nickname}', 'position', '${modalId}' );">${moreIcon}</a>`;
